...
public class League {
	 ...
	 * framework code to convert the data structure to the exchange format. For
	 * direct access of the matchdays, use {@link #getMatchday(int)}</b>
	 * 
	 */
	...
	/**
	 * Sets the list of matches. <b>The list is a 0-based java data structure.
	 * This method should not be used directly, but only by the framework code
	 * to convert the data structure to the exchange format. For direct access
	 * of the matchdays, use {@link #setMatchday(int, Matchday)}</b>
	 * 
	 */
